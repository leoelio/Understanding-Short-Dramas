...
每轮开发或演示后运行一次状态更新脚本，生成当前项目快照并追加运行记录：

```powershell
.\.venv\Scripts\python.exe scripts\update_project_status.py --summary "本轮目标" --change "完成内容" --next "下一步"
```

- `docs/PROJECT_STATUS.md`：当前完整状态，方便后续 AI 或团队成员快速接手。
- `docs/RUN_LOG.md`：按时间追加的运行记录。
