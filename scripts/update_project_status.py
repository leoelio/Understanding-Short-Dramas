...
def db_snapshot() -> dict:
    ...
    with sqlite3.connect(DB_PATH) as connection:
        ...
        return {
            ...
            "chat_messages": scalar(connection, "select count(*) from chat_messages"),
...
- 数据：{snapshot["dramas"]} 部短剧，{snapshot["episodes"]} 集，{snapshot["highlights"]} 个高光，{snapshot["reviewed_episodes"]} 集已复核，{snapshot["danmaku"]} 条弹幕，{snapshot["experience_configs"]} 条体验配置，{snapshot["ai_remixes"]} 条片尾 AI 二创，{snapshot["social_posts"]} 条社交动态，{snapshot["friend_requests"]} 条好友申请，{snapshot["chat_messages"]} 条聊天消息。
- 变更：
{change_lines}
- 下一步：
{next_lines}
