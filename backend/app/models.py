...
class DanmakuComment(Base):
    ...
    source_like_count = Column(Integer, default=0)
    review_status = Column(String(32), default="approved", index=True)
    risk_score = Column(Float, default=0)
    quality_score = Column(Float, default=0.7)
    spoiler_score = Column(Float, default=0)
    relevance_score = Column(Float, default=0.7)
    cluster_key = Column(String(96), default="", index=True)
    cluster_size = Column(Integer, default=1)
    suggested_time_sec = Column(Float, nullable=True)
    moderation_model_version = Column(String(64), default="")
    moderation_layers_json = Column(Text, default="{}")
    moderation_reason = Column(Text, default="")
    created_at = Column(DateTime, default=datetime.utcnow)
